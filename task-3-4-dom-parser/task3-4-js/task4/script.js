...
let ids = [188, 203, 206, 209, 213, 215, 228, 270, 271, 375, 395, 447, 452, 459, 463, 481, 500, 501]
...
const getStationsInfo = async () => {
    let parser = new DOMParser()
    ids.forEach(async sttID => {
        await fetch('../data_' + sttID + '.xml')
            .then(async res => {
                await res.text().then(async xml => {
                    xmlContent = xml
                    let xmlDOM = parser.parseFromString(xmlContent, 'application/xml')
                    let station = xmlDOM.querySelector('station');

                    const name = station.getAttribute('name')
                    const [lat, long] = station.getAttribute('geocode').split(',')
                    stations[sttID] = [name, parseFloat(lat), parseFloat(long)]
                })
            })
            .catch(_ => console.error(err));
    })
